...
$csv = $_POST["csv"];
$dat = $_POST["dtpicker"];
// echo "select * from payment where cardnumber ='$number' && csv= '$csv'&&exp ='$dat'";
...
if($rowcount>0)
{
    ...
    $stat = mysqli_query($con,"insert into foodorder values('','$user','$food')");
    ...
    if ($stat) {
        echo "<script> alert('food booked'); window.location='viewfood.php'</script>";
    } else {
        echo "<script> alert('Error encountered'); window.location='viewfood.php'</script>";
    }
}
else
{
    echo "<script> alert('invalid payment details'); window.location='viewfood.php'</script>";
}
}
?>
